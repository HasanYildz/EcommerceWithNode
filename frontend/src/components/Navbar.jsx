...
import { MdCategory, MdContacts, MdHomeFilled, MdShop2, MdMap } from 'react-icons/md';
...
const Navbar = ({ containerStyles }) => {
    return (
        <nav className={`${containerStyles}`}>
            ...
            <NavLink to="/sitemap" className={({ isActive }) => isActive ? "active_link" : ""}>
                <div className='flexCenter gap-x-1'><MdMap /> Sitemap</div>
            </NavLink>
        </nav>
    );
};
