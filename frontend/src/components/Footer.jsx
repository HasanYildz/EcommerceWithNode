...
const Footer = () => {
    return (
        <footer className="flexCenter pb-24 pt-20">
            <div className="max-padd_container flex w-full flex-col gap-14">
                <Link to="/" className="mb-10 font-bold text-2xl">Shoppee</Link>
                <div className="flex flex-col items-start justify-center gap-[10%] md:flex-row">
                    <div className="flex flex-wrap gap-16 sm:justify-between md:flex-1">
                        {FOOTER_LINKS.map((col) => (
                            <FooterColumn title={col.title} key={col.title}>
                                <ul className="flex flex-col gap-6 regular-14 text-gray-20">
                                    {col.links.map((link) => (
                ...
                <p className="text-center regular-14 text-gray-30">2024 Shopee | All right reserved.</p>
            </div>
        </footer>
    );
};
